...
class Piutang_telur extends Controller
{
    public function index(Request $r)
    {
        $data = [
            ...
            FROM tb_jurnal AS a
            LEFT JOIN (SELECT b.no_nota, b.urutan, b.id_post FROM invoice_telur AS b GROUP BY b.no_nota ) AS b ON concat('T-',b.no_nota)  = a.no_nota
    ...
    public function tambah_piutang(Request $r)
    {
        $data = [
            'count' => $r->count,
            'akun' => DB::table('tb_akun as a')->join('tb_permission_akun as b', 'a.id_akun', 'b.id_akun')->where('id_sub_menu_akun', '28')->get(),
    ...
    public function save_piutang_t(Request $r)
    {
        $no_nota = $r->no_nota;
        $kredit = $r->kredit;
        $tgl = $r->tgl;
        ...
        for ($x = 0; $x < count($no_nota); $x++) {
            ...
            if ($kredit[$x] - $kredit_bayar[$x] < 1) {
                ...
                DB::table('invoice_telur')->where('no_nota', $nota_invo[$x])->update($data_invoice);
            } else {
                # code...
            }
